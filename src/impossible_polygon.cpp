...
void polygon_init()
{
    ...
    program = InitShader("../shaders/objects/vshader_new.glsl", "../shaders/objects/fshader_new.glsl");
...
void polygon_display()
{
    ...
    auto get_adjusted_ball_pos = [&](float zDepth_draw)
    {
        ...
        if (k == 0)
            zDepth_ball = (float)N * zStep;
        else if (k >= N)
            zDepth_ball = 0.0f;
        else
            zDepth_ball = (float)(N - k) * zStep;
    ...
    // Bar 0 — THE DEPTH-TRICK BAR. Drawn as two half-segments at different z.
    {
        ...
        glUniform1f(bar_t_loc, 1.0f);

        // Swap topC and botC here to correct the Z-inversion of RotateZ(180)*RotateX(180)
        glUniform3fv(base_color_loc, 1, &botC.x);  // geometry top faces (verts 0..11) end up at physical bottom
        glDrawArrays(GL_TRIANGLES, 0, 12);
        glUniform3fv(base_color_loc, 1, &topC.x);  // geometry bottom faces (verts 12..23) end up at physical top
        glDrawArrays(GL_TRIANGLES, 12, 12);
        glUniform3fv(base_color_loc, 1, &tipC.x);  // geometry tip faces (verts 24..35)
        glDrawArrays(GL_TRIANGLES, 24, 12);
    }
...
void polygon_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    switch (key)
    {
        ...
        case GLFW_KEY_P:
            ...
            {
                fprintf(stderr,
                        "[path-editor] Slot 1 is procedural — the "
