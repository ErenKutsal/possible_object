...
//
//// rotation angles
//float penrose_angleX = -88.0;
//float penrose_angleY = 90.0;
//float penrose_angleZ = -2.5;
//
//// mouse control
...
float penrose_angleX = 0.0f;
float penrose_angleY = 0.0f;
float penrose_angleZ = 0.0f;
...
void penrose_keyCallback(GLFWwindow* win, int key, int, int action, int)
{
    if (action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        ...
        if (key == GLFW_KEY_R) { penrose_angleX = 0.0f; penrose_angleY = 0.0f; penrose_angleZ = 0.0f; }
        if (key == GLFW_KEY_ESCAPE) glfwSetWindowShouldClose(win, GL_TRUE);
    }
}
