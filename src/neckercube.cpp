...
void cube_buildScene()
{
    vec4 xColors[6] = {
        cubeBlue(), cubeMauve(),
        cubeCream(), cubeBlack(),
        cubeBlue(), cubeMauveDark()
    };
    vec4 yColors[6] = {
        cubeBlue(), cubeMauve(),
        cubeCreamDark(), cubeCreamDark(),
        cubeBlue(), cubeMauveDark()
    };
    vec4 zColors[6] = {
        cubeBlue(), cubeMauve(),
        cubeCream(), cubeBlack(),
        cubeBlueDark(), cubeMauveDark()
    };

    // Bars 0-1: bottom X, 2-3: bottom Z, 4-5: top X, 6-7: top Z, 8-11: verticals
    for (int i = 0; i < 2; i++)
        cube_buildBar(cube_positions + i * cube_vertsPerBar,
                      cube_colors + i * cube_vertsPerBar, xColors);
    for (int i = 2; i < 4; i++)
        cube_buildBar(cube_positions + i * cube_vertsPerBar,
                      cube_colors + i * cube_vertsPerBar, zColors);
    for (int i = 4; i < 6; i++)
        cube_buildBar(cube_positions + i * cube_vertsPerBar,
                      cube_colors + i * cube_vertsPerBar, xColors);
    for (int i = 6; i < 8; i++)
    ...
    for (int i = 8; i < 12; i++)
        cube_buildBar(cube_positions + i * cube_vertsPerBar,
                      cube_colors + i * cube_vertsPerBar, yColors);
}
...
void cube_mouseButtonCallback(GLFWwindow* window, int button, int action, int)
{
    if (button == GLFW_MOUSE_BUTTON_LEFT)
    {
        ...
        if (cube_isDragging)
            glfwGetCursorPos(window, &cube_mouseX, &cube_mouseY);
    }
}
...
void cube_scrollCallback(GLFWwindow*, double, double yoffset)
{
    cube_angleZ += (float)yoffset * 2.0f;
}
...
void cube_init()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, cube_barCount * cube_vertsPerBar * sizeof(vec4),
                 cube_positions, GL_STATIC_DRAW);

    cube_shaderProgram = InitShader("../shaders/vshader_simple.glsl",
                                    "../shaders/fshader_simple.glsl");
...
void cube_display()
{
    ...
    mat4 sceneRot = RotateY(cube_angleY)
                  * RotateX(cube_angleX)
                  * RotateZ(cube_angleZ);
