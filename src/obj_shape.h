...
struct ObjShape
{
    ...
    void setCustomLight(const vec3& p)
    {
        useCustomLight = true;
        customLightPos = p;
    }
    ...
    int ballStyle = 5;  // 5 = rainbow (default), 0 = purple glass
    GLint ballStyleLoc = -1;
    GLint ballTimeLoc = -1;
    GLint ballCameraPosLoc = -1;
    GLint ballModelLoc = -1;
    GLint ballViewLoc = -1;
    GLint ballProjLoc = -1;

    // Metallic mode + ball-reflection uniforms (fed into fshader_impossible)
    bool metallic = true;
    GLint metallicLoc = -1;
    GLint ballWorldPosLoc = -1;
    GLint ballRadiusLoc = -1;
    vec3 lastBallWorldPos = vec3(0.0f, 0.0f, 0.0f);  // 1-frame lag — imperceptible
    void setMetallic(bool m) { metallic = m; }

    void useMirrorBall(int style = 5)
    {
        ballStyle = style;
        ballShaderProgram = InitShader("../shaders/vshader_mirror.glsl", "../shaders/fshader_mirror.glsl");
        ballStyleLoc = glGetUniformLocation(ballShaderProgram, "u_ballStyle");
        ballTimeLoc = glGetUniformLocation(ballShaderProgram, "u_time");
        ballCameraPosLoc = glGetUniformLocation(ballShaderProgram, "cameraPos");
        ballModelLoc = glGetUniformLocation(ballShaderProgram, "model");
        ballViewLoc = glGetUniformLocation(ballShaderProgram, "view");
        ballProjLoc = glGetUniformLocation(ballShaderProgram, "projection");

        if (ballVao != 0)
        {
            glBindVertexArray(ballVao);
            glBindBuffer(GL_ARRAY_BUFFER, ballPosBuffer);
            GLint posLoc = glGetAttribLocation(ballShaderProgram, "vPosition");
            if (posLoc >= 0)
            {
            ...
            GLint colLoc = glGetAttribLocation(ballShaderProgram, "vColor");
            if (colLoc >= 0)
            {
    ...
    // ──────────────────────────────────────────────────────────────────────
    void init(const char* objPath, const ObjColorPalette& palette, const char* vshaderPath = nullptr,
              const char* fshaderPath = nullptr)
    {
        ...
        metallicLoc = glGetUniformLocation(shaderProgram, "uMetallic");
        ballWorldPosLoc = glGetUniformLocation(shaderProgram, "uBallWorldPos");
        ballRadiusLoc = glGetUniformLocation(shaderProgram, "uBallRadius");
    ...
    void setBallPath(const std::vector<PathWaypoint>& waypoints, float thickness, float ballRadius)
    {
        ballPath.clear();
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            vec3 right(fwd.y * up_ortho.z - fwd.z * up_ortho.y, fwd.z * up_ortho.x - fwd.x * up_ortho.z,
    ...
    {
        const int STACKS = 48, SLICES = 48;  // Higher resolution for smooth iridescent/glass marble shader
        ...
        glBindBuffer(GL_ARRAY_BUFFER, ballPosBuffer);
        glBufferData(GL_ARRAY_BUFFER, positions.size() * sizeof(vec4), positions.data(), GL_STATIC_DRAW);
        GLuint prog = ballShaderProgram ? ballShaderProgram : shaderProgram;
        GLint posLoc = glGetAttribLocation(prog, "vPosition");
        if (posLoc >= 0)
        {
            glEnableVertexAttribArray(posLoc);
            glVertexAttribPointer(posLoc, 4, GL_FLOAT, GL_FALSE, 0, (GLvoid*)0);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, ballColorBuffer);
        glBufferData(GL_ARRAY_BUFFER, colors.size() * sizeof(vec4), colors.data(), GL_STATIC_DRAW);
        GLint colLoc = glGetAttribLocation(prog, "vColor");
        if (colLoc >= 0)
        {
            glEnableVertexAttribArray(colLoc);
            glVertexAttribPointer(colLoc, 4, GL_FLOAT, GL_FALSE, 0, (GLvoid*)0);
    ...
    vec3 ballPositionFromSU() const
    {
        ...
        return vec3(center.x + y_local * seg->right.x + z_local * seg->up.x,
    ...
    static void quatSlerp(float q1x, float q1y, float q1z, float q1w, float q2x, float q2y, float q2z, float q2w,
                          float t, float& outX, float& outY, float& outZ, float& outW)
    {
    ...
    float extractIsoAxisAngleRad() const
    {
        ...
        vec3 cr(w.y * rwp.z - w.z * rwp.y, w.z * rwp.x - w.x * rwp.z, w.x * rwp.y - w.y * rwp.x);
    ...
    void enterRotating()
    {
        ...
        if (dot_iso >= 0.0f)
        {
            ...
            rotateTargetQ_x = 0.0f;
            rotateTargetQ_y = 0.0f;
            rotateTargetQ_z = 0.0f;
            rotateTargetQ_w = 1.0f;
        }
        else
        {
    ...
    void cancelLock() { solvePhase = SolvePhase::None; }
    ...
    mat4 getProjectionMatrix() const
    {
        ...
        return Ortho(-orthoSize * aspect, orthoSize * aspect, -orthoSize, orthoSize, 0.1f, 200.0f);
    }
    ...
    void display()
    {
        ...
        switch (solvePhase)
        {
            case SolvePhase::None:
            {
                ...
                if (isoAxisMisalignmentDeg() < AUTO_SNAP_ISO_TOL_DEG) enterFound();
                break;
            }
            ...
            case SolvePhase::Rotating:
            {
                ...
                if (t >= 1.0f)
                {
                    ...
                    rotateCurQ_x = rotateTargetQ_x;
                    rotateCurQ_y = rotateTargetQ_y;
                    rotateCurQ_z = rotateTargetQ_z;
                    rotateCurQ_w = rotateTargetQ_w;
                ...
                else
                {
                    ...
                    quatSlerp(rotateStartQ_x, rotateStartQ_y, rotateStartQ_z, rotateStartQ_w, rotateTargetQ_x,
                              rotateTargetQ_y, rotateTargetQ_z, rotateTargetQ_w, et, rotateCurQ_x, rotateCurQ_y,
                              rotateCurQ_z, rotateCurQ_w);
                }
                lockGlow = 0.18f;
                break;
            }
        ...
        float aspect = (screen_w > 0 && screen_h > 0) ? (float)screen_w / (float)screen_h : 1.0f;
        mat4 projection = Ortho(-orthoSize * aspect, orthoSize * aspect, -orthoSize, orthoSize, 0.1f, 200.0f);
        ...
        vec3 lightPos = useCustomLight ? customLightPos : vec3(eye.x * 0.8f, eye.y * 1.6f, eye.z * 1.2f);
        ...
        if (solvePhase == SolvePhase::None || solvePhase == SolvePhase::Found)
        {
            ...
            float t = (PROXIMITY_NONE_DEG - mis) / (PROXIMITY_NONE_DEG - PROXIMITY_FULL_DEG);
            if (t < 0.0f) t = 0.0f;
            if (t > 1.0f) t = 1.0f;
        ...
        if (ballActive && drawBallMesh)
        {
            ...
            if (solvePhase == SolvePhase::Rotating)
            {
                rotPartOnly = matFromQuat(rotateCurQ_x, rotateCurQ_y, rotateCurQ_z, rotateCurQ_w);
            }
            else
            {
                rotPartOnly = RotateY(angleY) * RotateX(angleX) * RotateZ(angleZ);
            }
            mat4 ballModel = Translate(world_ball_pos.x, world_ball_pos.y, world_ball_pos.z) * rotPartOnly * ballScale;

            if (ballShaderProgram != 0)
            {
                glUseProgram(ballShaderProgram);
                glUniformMatrix4fv(ballModelLoc, 1, GL_FALSE, &ballModel.d[0].x);
                glUniformMatrix4fv(ballViewLoc, 1, GL_FALSE, &view.d[0].x);
                glUniformMatrix4fv(ballProjLoc, 1, GL_FALSE, &projection.d[0].x);
                if (ballStyleLoc >= 0) glUniform1i(ballStyleLoc, ballStyle);
                if (ballTimeLoc >= 0) glUniform1f(ballTimeLoc, (float)now);
                if (ballCameraPosLoc >= 0) glUniform3fv(ballCameraPosLoc, 1, &eye.x);
            }
            else
            {
    ...
    void scroll(double yoffset) { angleZ += (float)yoffset * 2.0f; }
    ...
    void key(GLFWwindow* win, int keyCode, int action)
    {
        ...
        bool wasd_key =
            (keyCode == GLFW_KEY_W || keyCode == GLFW_KEY_A || keyCode == GLFW_KEY_S || keyCode == GLFW_KEY_D);
        if (wasd_key)
        {
        ...
        if (keyCode == GLFW_KEY_S)
        {
            ...
            if (solvePhase == SolvePhase::None || solvePhase == SolvePhase::Found) enterRotating();
        }
