...
void main()
{
    // Light-blue blueprint field (pale sky blue)
    vec3 baseCol = vec3(0.60, 0.76, 0.90);

    // Radial gradient glow centered on the screen (brighter centre)
    float dCenter = length(vNdc);
    float glow = exp(-dCenter * dCenter * 1.2);
    vec3 glowCol = vec3(0.74, 0.87, 0.97);
    ...
    if (uAmount > 0.001)
    {
        ...
        // Medium-blue drafting lines that read against the light field
        vec3 gridColor = vec3(0.20, 0.40, 0.66); // medium blueprint blue
        vec3 guideColor = vec3(0.16, 0.46, 0.60); // blueprint cyan-teal

        // Combine layers
        vec3 blueprintLayer = col;
        blueprintLayer = mix(blueprintLayer, gridColor, grid_line * 0.45);
        blueprintLayer = mix(blueprintLayer, guideColor, dotted_circle * 0.35);
        blueprintLayer = mix(blueprintLayer, guideColor * 0.9, dotted_ray * 0.28);

        // Subtle blueprint sweep highlight (darkens slightly as it passes)
        float sweep = sin(uv.x * 2.0 - uTime * 0.5) * 0.5 + 0.5;
        blueprintLayer = mix(blueprintLayer, gridColor, grid_line * sweep * 0.10);
